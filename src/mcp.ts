...
import { get64bitRandom } from './lib/get64BitRandom.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);

const sintImage = await toFile(
  fs.createReadStream(path.resolve(__dirname, '../src/assets/sint.jpeg')),
  null,
  { type: 'image/png' }
);
      ...
      try {
        ...
        const images: File[] = await Promise.all(
          res.data.map(async (image) => {
            ...
            const fileName = `image_${get64bitRandom()}.jpeg`;
        ...
        const imagesUrls = images.map(
          (i) =>
            `${env.MINIO_BUCKET_URL}/${env.MINIO_BUCKET_NAME}/public/${i.name}`
        );
